...
(() => {
  ...
  function buildToolbar() {
    ...
    // reach, and export moves to the panel head.
    const exportButtons = [];
  ...
  function saveAnnotations() {
    ...
    try {
      ...
      // Silent until now. A capture on a coarse pointer is the whole viewport
      // rather than a hand-framed corner of it, so the store fills faster than
      // it used to and a note that was never written down should say so.
      console.warn('Annotator storage save error', err);
      showToast('Uxnote: this browser has no room left to store notes');
    }
  }
