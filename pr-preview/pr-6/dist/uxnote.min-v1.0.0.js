(()=>{var Eo=Object.defineProperty,Lo=Object.defineProperties;var Ao=Object.getOwnPropertyDescriptors;var Pe=Object.getOwnPropertySymbols;var To=Object.prototype.hasOwnProperty,So=Object.prototype.propertyIsEnumerable;var Oe=(p,g,C)=>g in p?Eo(p,g,{enumerable:!0,configurable:!0,writable:!0,value:C}):p[g]=C,Rt=(p,g)=>{for(var C in g||(g={}))To.call(g,C)&&Oe(p,C,g[C]);if(Pe)for(var C of Pe(g))So.call(g,C)&&Oe(p,C,g[C]);return p},Be=(p,g)=>Lo(p,Ao(g));(()=>{if(window.Uxnote)return;const p=document.currentScript||Array.from(document.querySelectorAll("script")).find(t=>(t.getAttribute("src")||"").includes("annotator.js")),g=t=>p?p.getAttribute(t):null,C=`${location.protocol}//${location.host}`,Fe=p&&(p.dataset.mailto||p.dataset.email||p.dataset.to)||"",gt=g("isToolVisibleAtFirstLaunch")||g("istoolvisibleatfirstlaunch")||p&&(p.dataset.isToolVisibleAtFirstLaunch||p.dataset.istoolvisibleatfirstlaunch),wt=g("isToolOnTopAtLaunch")||g("istoolontopatlaunch")||p&&(p.dataset.isToolOnTopAtLaunch||p.dataset.istoolontopatlaunch),He=p&&(p.dataset.hiddentoolbydefault||p.dataset.hidden||p.dataset.collapsed||p.dataset.startHidden||""),Ue=g("colorForHighlight")||g("colorForHighligh")||p&&(p.dataset.colorForHighlight||p.dataset.colorForHighligh),Pt=g("colorForTextHighligh")||g("colorForTextHighlight")||p&&(p.dataset.colorForTextHighligh||p.dataset.colorForTextHighlight),Ot=g("colorForElementHighlight")||g("colorForElementHighligh")||p&&(p.dataset.colorForElementHighlight||p.dataset.colorForElementHighligh),Bt="#4e9cf6",K=nt(Ue||Ot||Pt||Bt,Bt),De=nt(Pt||K,K),je=nt(Ot||K,K),xt={text:kt(De,{overlayAlpha:.7,softAlpha:.18,softerAlpha:.08}),element:kt(je,{overlayAlpha:.35,softAlpha:.12,softerAlpha:.04}),screenshot:kt(K,{overlayAlpha:.35,softAlpha:.12,softerAlpha:.04})};let k=wt!=null?B(wt,!1)?"top":"bottom":p&&p.dataset.position||"bottom";const Ft="wn-toolbar-pos",G=p&&(p.dataset.dock||p.dataset.layout)||"",Ht=`uxnote:site:${C}`,Ut=`uxnote:import-files:${C}`,Dt=`uxnote:hidden:${C}`,bt=`uxnote:pending:${C}`,jt=(p&&p.dataset.serverUrl||"").trim().replace(/\/+$/,""),E=jt?{url:jt,apiKey:p&&p.dataset.serverApiKey||""}:null,Kt=B(p&&p.dataset.jsonExport,!0),yt=B(p&&p.dataset.jsonImport,!0),Ke=g("isBackdropVisible")||g("isbackdropvisible")||g("backdropVisible")||g("backdropvisible")||p&&(p.dataset.isBackdropVisible||p.dataset.isbackdropvisible||p.dataset.backdropVisible||p.dataset.backdropvisible||p.dataset.dim||p.dataset.dimpage||p.dataset.dimmer||p.dataset.overlay||p.dataset.dimLevel||p.dataset.dimlevel||p.dataset.dimstrength),Xe=.2,Xt=B(Ke,!0),r={mode:null,annotations:[],importFiles:[],markers:{},highlightSpans:{},elementTargets:{},outlineBox:null,toolbar:null,panel:null,visibilityToggle:null,commentModal:null,dialogModal:null,importModal:null,exportModal:null,markerLayer:null,colors:xt,customPosition:!1,dimEnabled:Xt,dimOpacity:Xt?Xe:0,dimOverlay:null,filters:{query:""},hidden:!1,missingObserver:null,missingRetryTimer:null,layoutObserver:null,layoutTimer:null,toast:null,toastTimer:null},qt=window.matchMedia?window.matchMedia("(max-width: 640px)"):null;function Q(){return qt?qt.matches:window.innerWidth<=640}function Yt(){const t=fn();t&&(k=t);const e=hn(),n=gt!=null?!B(gt,!0):null;r.hidden=e!==null?e:n!==null?n:B(He,!1),yt&&(r.importFiles=yn()),Vt(),wn(),qe(),Ye(),Ve(),ot(r.hidden),E?It():vn(),V(),Nt(),fe(),Yn(),E||Te(),dn()}function Vt(){const t=getComputedStyle(document.body);r.basePadding={top:parseFloat(t.paddingTop)||0,right:parseFloat(t.paddingRight)||0,bottom:parseFloat(t.paddingBottom)||0,left:parseFloat(t.paddingLeft)||0}}function qe(){const t=document.createElement("style");t.setAttribute("data-wn-style","annotator"),t.textContent=`
    ...
    `,document.head.appendChild(t)}function Ye(){const t=document.createElement("div");t.className=`wn-annot-toolbar wn-annotator wn-pos-${k}`;const e=w=>{const u=document.createElement("button");return u.className="wn-annot-btn wn-annotator",u.setAttribute("data-action",w.action),w.mode&&u.setAttribute("data-mode",w.mode),u.setAttribute("data-tip",w.tip),u.innerHTML=w.icon,u},n=w=>{const u=document.createElement("div");return u.className="wn-annot-group wn-annotator",w.forEach(x=>u.appendChild(e(x))),u},o=()=>{const w=document.createElement("div");return w.className="wn-annot-spacer wn-annotator",w},a=document.createDocumentFragment(),i=document.createElement("div");i.className="wn-annot-logo wn-annotator",i.innerHTML=no(),a.appendChild(i);const s=[{action:"mode",mode:"text",tip:"Highlight text",icon:Ce()},{action:"mode",mode:"element",tip:"Annotate an element",icon:oo()}];Re()&&s.push({action:"mode",mode:"screenshot",tip:"Capture a region",icon:io()});const l=[];yt&&l.push({action:"import",tip:"Import JSON",icon:ao()}),Kt&&l.push({action:"export",tip:"Export JSON",icon:ro()});const c=[{action:"toggle-pos",tip:"Toolbar top / bottom",icon:co()},{action:"toggle-panel",tip:"Show / hide annotations",icon:lo()}];a.appendChild(o()),a.appendChild(n(s)),l.length&&(a.appendChild(o()),a.appendChild(n(l))),a.appendChild(o()),a.appendChild(n(c)),t.appendChild(a),document.body.appendChild(t),r.toolbar=t;const d=document.createElement("div");d.className="wn-annot-panel wn-annotator",d.innerHTML=`
