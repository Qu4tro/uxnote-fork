(() => {
  const translations = {
    en: {
      ...
      features_card2_title: 'Editing & search',
      features_card2_body: 'Edit or delete a comment from the panel, and find a note again by keyword.',
      ...
      install_screenshots_title: 'Region screenshots',
      install_screenshots_lead: 'Load snapdom before Uxnote, on the same page, and the toolbar offers a camera. Frame a region and it becomes an annotation:',
      ...
      builder_json_export_desc: 'JSON export button and dialog.',
      builder_json_import_desc: 'JSON import button and dialog.',
      builder_mail_export_desc: 'Send by mail button. false also silences data-mailto.',
      builder_theme_desc: 'Theme of the toolbar, panel, and dialogs. auto follows the system.',
      builder_server_desc: 'Server that stores the annotations.',
      builder_server_key_desc: 'Key sent with every request. It is visible in the page source.',
      ...
      install_opt_json_export_body: 'Keeps or removes the JSON export button and its dialog. The email handoff has its own option. Defaults to true.<br> Example:<br> data-json-export="false".',
      install_opt_json_import_body: 'Keeps or removes the JSON import and its list of imported files. Defaults to true.<br> Example:<br> data-json-import="false".',
      install_opt_mail_export_body: 'Keeps or removes the button that hands the annotations to the mail client, and with it the meaning of data-mailto. Defaults to true.<br> Example:<br> data-mail-export="false".',
      install_opt_theme_body: 'Chooses the theme of the widget. auto follows the system setting and changes with it; light and dark hold one theme whatever the system says. Defaults to auto.<br> Example:<br> data-theme="dark".',
      install_opt_server_body: 'Names the server that keeps the annotations of this site, over the protocol of PROTOCOL.md. Leave it out and the annotations stay in the browser. The api key travels as the X-Uxnote-Key header, and anybody who reads the page reads it, so it keeps a passer-by out and nothing more.<br> Example:<br> data-server-url="http://localhost:8123" data-server-api-key="review-key".',
    ...
    fr: {
      ...
      features_card2_title: 'Édition & recherche',
      features_card2_body: 'Modifiez ou supprimez un commentaire depuis le panneau, et retrouvez une note par mot-clé.',
      features_card3_title: 'Import / Export',
      ...
      install_screenshots_title: 'Captures de zone',
      install_screenshots_lead: 'Chargez snapdom avant Uxnote, sur la même page, et la barre d’outils propose un appareil photo. Cadrez une zone et elle devient une annotation :',
      ...
      builder_json_export_desc: 'Bouton et fenêtre d’export JSON.',
      builder_json_import_desc: 'Bouton et fenêtre d’import JSON.',
      builder_mail_export_desc: 'Bouton d’envoi par email. false rend aussi data-mailto sans effet.',
      builder_theme_desc: 'Thème de la barre, du panneau et des fenêtres. auto suit le système.',
      builder_server_desc: 'Serveur qui conserve les annotations.',
      builder_server_key_desc: 'Clé envoyée à chaque requête. Elle est visible dans le source de la page.',
      ...
      install_opt_json_export_body: 'Conserve ou retire le bouton d’export JSON et sa fenêtre. L’envoi par email a sa propre option. Vaut true par défaut.<br> Exemple :<br> data-json-export="false".',
      install_opt_json_import_body: 'Conserve ou retire l’import JSON et sa liste de fichiers importés. Vaut true par défaut.<br> Exemple :<br> data-json-import="false".',
      install_opt_mail_export_body: 'Conserve ou retire le bouton qui confie les annotations au client mail, et avec lui l’effet de data-mailto. Vaut true par défaut.<br> Exemple :<br> data-mail-export="false".',
      install_opt_theme_body: 'Choisit le thème de l’outil. auto suit le réglage du système et change avec lui ; light et dark fixent un thème quel que soit le système. Vaut auto par défaut.<br> Exemple :<br> data-theme="dark".',
      install_opt_server_body: 'Désigne le serveur qui conserve les annotations de ce site, via le protocole de PROTOCOL.md. Sans cet attribut, les annotations restent dans le navigateur. La clé voyage dans l’en-tête X-Uxnote-Key, et toute personne qui lit la page la lit : elle écarte un visiteur de passage, rien de plus.<br> Exemple :<br> data-server-url="http://localhost:8123" data-server-api-key="review-key".',
