...
- `data-toolbar-density`, `compact` or `normal`, with a control for it in the demo's settings grid. It names how much height the toolbar spends where a mouse is driving; `normal` is the size a finger is given, on a screen no finger reaches.
### Changed
...
- The toolbar stands 42px tall where a mouse is driving rather than 66px: a 32px control between 4px of padding, with the gaps, the spacers and the floating eye cut to match. The bar is fixed over the page under review, so the 24px it gives up are 24px of that page a reviewer can see again. Where the pointer is coarse nothing moved: the 44px reach is not on the density switch.
