...
### Changed
- `data-mail-export` is gone. The mail handoff is switched by the address it sends to: name a `data-mailto` and the toolbar carries the button, name none and there is none. The two attributes could disagree, and the pairing that read worst was the default one -- no address and the button there anyway, opening a draft addressed to nobody. An address the widget cannot parse counts as no address.
